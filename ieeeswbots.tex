...
\section{How to create Bots and where to host them}
	...
	Pretending isn't simply restricted to bots attempting to disguise themselves as humans. Some bots allow for human intervention when a user has a question or task that the bot is unable to provide an appropriate response to.  This switch must be handled correctly, as can lead to the same distrust if users believe they are speaking to a bot and instead realize it's a human.
